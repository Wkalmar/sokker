							...
							htmlToJson.parse(body, ['.panel-body .well',
								...
								function ($players) {
									...
									$players.forEach((item)=> {
										const $player = $(item);
										const $strng = $player.find('.col-md-6.col-sm-5.col-xs-12.small').find("strng").find("strong");

										const id = $player.find("#playerCell").find("div").first().text().trim();

										const currentPrice = $($strng[1]).text().trim();
										const endOfTrade = $($strng[2]).text().trim();
										...
										DATA.push({
											id, name, age, stamina, keeper, pace, defender, technique, playmaker, passing, striker, currentPrice, endOfTrade
										});
									});
									return DATA;
								}])
